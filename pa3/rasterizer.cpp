...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    auto v = t.toVector4();
    int min_x = (int)std::floor(std::min(v[0].x(), std::min(v[1].x(), v[2].x())));
    int max_x = (int)std::ceil(std::max(v[0].x(), std::max(v[1].x(), v[2].x())));
    int min_y = (int)std::floor(std::min(v[0].y(), std::min(v[1].y(), v[2].y())));
    int max_y = (int)std::ceil(std::max(v[0].y(), std::max(v[1].y(), v[2].y())));
